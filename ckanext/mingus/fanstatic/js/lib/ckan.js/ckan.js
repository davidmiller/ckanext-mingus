...
(function(my) {
    ...
    my.Client.prototype.action = function(name, data) {
	if (data == undefined){
	    data = {};
	}
  ...
  // make an AJAX request
    my.Client.prototype._ajax = function(options) {
  ...
  // Primarily for use by Recline backend below
	...
	this.action('datastore_search', actualQuery).then(
            function(data){
	...
	var q = $.Deferred();
	this.action('datastore_search_sql', {sql: sql}).then(
            function(results){
              // map ckan types to our usual types ...
		var fields = _.map(results.result.fields, function(field) {
                    field.type = field.type in my.ckan2JsonTableSchemaTypes ? my.ckan2JsonTableSchemaTypes[field.type] : field.type;
  ...
  // list all the resources with an entry in the DataStore
    my.Client.prototype.datastoreResources = function() {
	return this.action('datastore_search', {
  ...
  // =========
    ...
    var _browserRequest = function(options) {
	var q = $.Deferred();
	var self = this;
...
(function(my) {
  ...
  // ### fetch
	...
	return wrapper.datastoreQuery(queryObj)
    };
